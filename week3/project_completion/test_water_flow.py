...
def test_pressure_loss_from_fittings() -> None:
    ...
    assert pressure_loss_from_fittings(1.75, 5) == approx(-0.306, abs=0.01)
    assert pressure_loss_from_fittings(2.0, 3) == approx(-0.240, abs=0.01)
...
def test_reynolds_number() -> None:
    ...
    assert reynolds_number(0.1, 1.0) == approx(99660, rel=0.01)
    assert reynolds_number(0.2, 2.0) == approx(398640, rel=0.01)
    assert reynolds_number(0.05, 3.0) == approx(149490, rel=0.01)
...
def test_kpa_to_psi() -> None:
    ...
    assert kpa_to_psi(158.7) == approx(23.017, abs=0.01)
